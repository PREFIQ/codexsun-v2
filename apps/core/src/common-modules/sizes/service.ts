...
import type { SizeRecord, SizeCreateInput, SizeUpdateInput } from "./contracts.js";
import type { SizeRepository } from "./repository.js";

export class SizeService {
  constructor(private readonly repository: SizeRepository) {}

  async list(tenantId: string): Promise<SizeRecord[]> {
    if (!tenantId) throw AppError.validation("tenantId is required");
    return this.repository.list(tenantId);
  }

  async getById(tenantId: string, id: string): Promise<SizeRecord> {
    const record = await this.repository.getById(tenantId, id);
    if (!record) throw AppError.notFound("Size not found");
    return record;
  }

  async create(input: SizeCreateInput): Promise<SizeRecord> {
  ...
  async archive(tenantId: string, id: string): Promise<void> {
    ...
    if (!existing.isActive) throw AppError.conflict("Size is already archived");
  ...
  async restore(tenantId: string, id: string): Promise<void> {
    ...
    if (!existing) throw AppError.notFound("Size not found");
    if (existing.isActive) throw AppError.conflict("Size is already active");
