...
export const paymentEntryStorageMigration: BillingMigration = {
  id: "014_payments",
  description: "Tenant payment owned documents, items, allocations, comments, activities, and compliance operations",
  up: async (db) => {
    await db.execute(`
      CREATE TABLE IF NOT EXISTS payments (
        ...
        UNIQUE KEY uq_payments_entry (tenant_id, entry_id),
        UNIQUE KEY uq_payments_no (tenant_id, document_no),
        KEY ix_payments_date (tenant_id, document_date, id),
        KEY ix_payments_party (tenant_id, party_name),
        KEY ix_payments_status (tenant_id, status, is_active)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS payment_items (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_payment_items_line (tenant_id, entry_id, line_id),
        KEY ix_payment_items_parent (tenant_id, entry_id, sort_order)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS payment_allocations (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_payment_allocations_allocation (tenant_id, entry_id, allocation_id),
        KEY ix_payment_allocations_parent (tenant_id, entry_id, sort_order),
        KEY ix_payment_allocations_document (tenant_id, document_type, document_no)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS payment_comments (
        ...
        body TEXT NOT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uq_payment_comments_comment (tenant_id, entry_id, comment_id),
        KEY ix_payment_comments_parent (tenant_id, entry_id, id)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS payment_activities (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uq_payment_activities_activity (tenant_id, entry_id, activity_id),
        KEY ix_payment_activities_parent (tenant_id, entry_id, id)
      )
    `);

    await db.execute(`
      CREATE TABLE IF NOT EXISTS payment_compliances (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
        tenant_id VARCHAR(120) NOT NULL,
        entry_id VARCHAR(80) NOT NULL,
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_payment_compliances_operation (tenant_id, entry_id, operation_id),
        KEY ix_payment_compliances_parent (tenant_id, entry_id, id),
        KEY ix_payment_compliances_provider (provider, environment, operation, status)
      )
    `);
  },
};

export const paymentMigrations = [paymentEntryStorageMigration];
