...
export async function registerSalesRoutes(app: FastifyInstance, ctx: BillingRouteContext) {
  for (const basePath of ["/billing/entries", "/core/entries"]) {
    for (const routeKind of routeKinds) {
      registerSalesRouteSet(app, ctx, `${basePath}/${routeKind}`);
    }
  }
}

function registerSalesRouteSet(app: FastifyInstance, ctx: BillingRouteContext, routePath: string) {
