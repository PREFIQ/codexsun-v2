...
export async function registerPurchaseRoutes(app: FastifyInstance, ctx: BillingRouteContext) {
  for (const basePath of ["/billing/entries", "/core/entries"]) {
    for (const routeKind of routeKinds) {
      registerPurchaseRouteSet(app, ctx, `${basePath}/${routeKind}`);
    }
  }
}

function registerPurchaseRouteSet(app: FastifyInstance, ctx: BillingRouteContext, routePath: string) {
