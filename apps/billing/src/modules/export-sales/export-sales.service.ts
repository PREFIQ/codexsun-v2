...
const moduleDefinition = exportSalesEntryModule;
...
export async function requireTenantSession(app: FastifyInstance, request: FastifyRequest & { tenantId?: string }, ctx: BillingRouteContext, mutate = false) {
  ...
  if (mutate) {
    await ctx.guardActiveTenant(app, request.tenantId);
    await ctx.guardFeatureEnabled(app, request.tenantId, "business.billing");
  }
  return session;
}

export async function listEntries(app: FastifyInstance, tenantId: string, kind: EntryKind) {
...
export async function getEntry(app: FastifyInstance, tenantId: string, kind: EntryKind, id: string) {
  const [rows] = await app.masterDbPool.execute<EntryDocumentRow[]>(
    `SELECT * FROM export_sales
...
export async function upsertEntry(app: FastifyInstance, tenantId: string, kind: EntryKind, input: EntryInput, actorEmail: string) {
  ...
  if (moduleDefinition.requiresLines && lines.length === 0) {
...
export async function setEntryActive(app: FastifyInstance, tenantId: string, kind: EntryKind, id: string, active: boolean, actorEmail: string) {
...
export async function addEntryComment(app: FastifyInstance, tenantId: string, kind: EntryKind, id: string, body: string | undefined, actorEmail: string) {
  const comment = String(body ?? "").trim();
  if (!comment) throw AppError.validation("Comment is required");
  const entry = await getEntry(app, tenantId, kind, id);
  await app.masterDbPool.execute(
    `INSERT INTO export_sales_comments (tenant_id, entry_id, comment_id, author_email, body)
     VALUES (?, ?, ?, ?, ?)`,
    [tenantId, entry.entryId, newId(), actorEmail, comment],
  );
  await addActivity(app, tenantId, entry.entryId, "comment", actorEmail, "Comment added", { body: comment });
  return getEntry(app, tenantId, kind, entry.entryId);
}

export async function recordEntryToolAction(app: FastifyInstance, tenantId: string, kind: EntryKind, id: string, input: { tool?: string; value?: string }, actorEmail: string) {
  const tool = clean(input.tool) ?? "entry-tool";
  const value = clean(input.value);
  const entry = await getEntry(app, tenantId, kind, id);
  const message = entryToolMessage(tool, value);
  await addActivity(app, tenantId, entry.entryId, "tool", actorEmail, message, { tool, value });
  return getEntry(app, tenantId, kind, entry.entryId);
}

export async function runEntryComplianceAction(app: FastifyInstance, tenantId: string, kind: EntryKind, id: string, action: string, input: Record<string, unknown> | undefined, actorEmail: string) {
  if (!moduleDefinition.supportsCompliance) throw AppError.validation("Compliance generation is available for sales-style entries only");
  const entry = await getEntry(app, tenantId, kind, id);
  const result = await runWhiteBooksCompliance(app, tenantId, entry, action, input, actorEmail);
  return { entry: await getEntry(app, tenantId, kind, entry.entryId), document: result.document, provider: result.provider };
}
async function findEntryRow(app: FastifyInstance, tenantId: string, kind: EntryKind, entryId: string) {
  const [rows] = await app.masterDbPool.execute<EntryDocumentRow[]>(
    `SELECT * FROM export_sales WHERE tenant_id = ? AND entry_type = ? AND entry_id = ? LIMIT 1`,
...
async function replaceLines(app: FastifyInstance, tenantId: string, entryId: string, lines: Required<EntryLineInput>[]) {
  await app.masterDbPool.execute("DELETE FROM export_sales_items WHERE tenant_id = ? AND entry_id = ?", [tenantId, entryId]);
  for (const [index, line] of lines.entries()) {
    ...
    await app.masterDbPool.execute(
      `INSERT INTO export_sales_items (
...
async function replaceAllocations(app: FastifyInstance, tenantId: string, entryId: string, allocations: Required<EntryAllocationInput>[]) {
  await app.masterDbPool.execute("DELETE FROM export_sales_allocations WHERE tenant_id = ? AND entry_id = ?", [tenantId, entryId]);
  for (const [index, allocation] of allocations.entries()) {
    const balance = round(allocation.previousBalance - allocation.allocatedAmount);
    await app.masterDbPool.execute(
      `INSERT INTO export_sales_allocations (
...
async function hydrateEntry(app: FastifyInstance, row: EntryDocumentRow) {
  const [lineRows] = await app.masterDbPool.execute<Array<Record<string, unknown>>>(
    "SELECT * FROM export_sales_items WHERE tenant_id = ? AND entry_id = ? ORDER BY sort_order ASC, id ASC",
    [row.tenant_id, row.entry_id],
  );
  const [allocationRows] = await app.masterDbPool.execute<Array<Record<string, unknown>>>(
    "SELECT * FROM export_sales_allocations WHERE tenant_id = ? AND entry_id = ? ORDER BY sort_order ASC, id ASC",
    [row.tenant_id, row.entry_id],
  );
  const [commentRows] = await app.masterDbPool.execute<Array<Record<string, unknown>>>(
    "SELECT * FROM export_sales_comments WHERE tenant_id = ? AND entry_id = ? ORDER BY id ASC",
    [row.tenant_id, row.entry_id],
  );
  const [activityRows] = await app.masterDbPool.execute<Array<Record<string, unknown>>>(
    "SELECT * FROM export_sales_activities WHERE tenant_id = ? AND entry_id = ? ORDER BY id ASC",
    [row.tenant_id, row.entry_id],
  );
  return {
...
async function nextDocumentNo(app: FastifyInstance, tenantId: string, kind: EntryKind) {
  const prefix = moduleDefinition.documentPrefix;
  const [rows] = await app.masterDbPool.execute<Array<{ count_value: number | string }>>(
    "SELECT COUNT(*) AS count_value FROM export_sales WHERE tenant_id = ? AND entry_type = ?",
    [tenantId, kind],
  );
...
async function addActivity(app: FastifyInstance, tenantId: string, entryId: string, type: string, actorEmail: string, message: string, payload: Record<string, unknown>) {
  await app.masterDbPool.execute(
    `INSERT INTO export_sales_activities (tenant_id, entry_id, activity_id, activity_type, actor_email, message, payload_json)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
async function runWhiteBooksCompliance(app: FastifyInstance, tenantId: string, entry: Awaited<ReturnType<typeof hydrateEntry>>, action: string, input: Record<string, unknown> | undefined, actorEmail: string) {
  ...
  await app.masterDbPool.execute(
    `INSERT INTO export_sales_compliances
  ...
  try {
    ...
    await app.masterDbPool.execute(
      `UPDATE export_sales_compliances SET status = 'success', response_json = ? WHERE tenant_id = ? AND operation_id = ?`,
    ...
    await app.masterDbPool.execute(
      `UPDATE export_sales_compliances SET status = 'failed', error_message = ? WHERE tenant_id = ? AND operation_id = ?`,
...
function entryLabel(_kind: EntryKind) { return moduleDefinition.label; }

function partyLabel(_kind: EntryKind) { return moduleDefinition.partyLabel; }

function defaultPartyType(_kind: EntryKind) { return moduleDefinition.defaultPartyType; }
