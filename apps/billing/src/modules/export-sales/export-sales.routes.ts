...
export async function registerExportSalesRoutes(app: FastifyInstance, ctx: BillingRouteContext) {
  for (const basePath of ["/billing/entries", "/core/entries"]) {
    for (const routeKind of routeKinds) {
      registerExportSalesRouteSet(app, ctx, `${basePath}/${routeKind}`);
    }
  }
}

function registerExportSalesRouteSet(app: FastifyInstance, ctx: BillingRouteContext, routePath: string) {
