...
export const quotationEntryStorageMigration: BillingMigration = {
  id: "009_quotations",
  description: "Tenant quotation owned documents, items, allocations, comments, activities, and compliance operations",
  up: async (db) => {
    await db.execute(`
      CREATE TABLE IF NOT EXISTS quotations (
        ...
        entry_type VARCHAR(30) NOT NULL DEFAULT 'quotation',
        ...
        UNIQUE KEY uq_quotations_entry (tenant_id, entry_id),
        UNIQUE KEY uq_quotations_no (tenant_id, document_no),
        KEY ix_quotations_date (tenant_id, document_date, id),
        KEY ix_quotations_party (tenant_id, party_name),
        KEY ix_quotations_status (tenant_id, status, is_active)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS quotation_items (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_quotation_items_line (tenant_id, entry_id, line_id),
        KEY ix_quotation_items_parent (tenant_id, entry_id, sort_order)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS quotation_allocations (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_quotation_allocations_allocation (tenant_id, entry_id, allocation_id),
        KEY ix_quotation_allocations_parent (tenant_id, entry_id, sort_order),
        KEY ix_quotation_allocations_document (tenant_id, document_type, document_no)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS quotation_comments (
        ...
        body TEXT NOT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uq_quotation_comments_comment (tenant_id, entry_id, comment_id),
        KEY ix_quotation_comments_parent (tenant_id, entry_id, id)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS quotation_activities (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uq_quotation_activities_activity (tenant_id, entry_id, activity_id),
        KEY ix_quotation_activities_parent (tenant_id, entry_id, id)
      )
    `);

    await db.execute(`
      CREATE TABLE IF NOT EXISTS quotation_compliances (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
        tenant_id VARCHAR(120) NOT NULL,
        entry_id VARCHAR(80) NOT NULL,
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_quotation_compliances_operation (tenant_id, entry_id, operation_id),
        KEY ix_quotation_compliances_parent (tenant_id, entry_id, id),
        KEY ix_quotation_compliances_provider (provider, environment, operation, status)
      )
    `);
  },
};

export const quotationMigrations = [quotationEntryStorageMigration];
