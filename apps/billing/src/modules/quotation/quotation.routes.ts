...
export async function registerQuotationRoutes(app: FastifyInstance, ctx: BillingRouteContext) {
  for (const basePath of ["/billing/entries", "/core/entries"]) {
    for (const routeKind of routeKinds) {
      registerQuotationRouteSet(app, ctx, `${basePath}/${routeKind}`);
    }
  }
}

function registerQuotationRouteSet(app: FastifyInstance, ctx: BillingRouteContext, routePath: string) {
