...
export async function registerReceiptRoutes(app: FastifyInstance, ctx: BillingRouteContext) {
  for (const basePath of ["/billing/entries", "/core/entries"]) {
    for (const routeKind of routeKinds) {
      registerReceiptRouteSet(app, ctx, `${basePath}/${routeKind}`);
    }
  }
}

function registerReceiptRouteSet(app: FastifyInstance, ctx: BillingRouteContext, routePath: string) {
