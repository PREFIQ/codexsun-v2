...
export const receiptEntryStorageMigration: BillingMigration = {
  id: "011_receipts",
  description: "Tenant receipt owned documents, items, allocations, comments, activities, and compliance operations",
  up: async (db) => {
    await db.execute(`
      CREATE TABLE IF NOT EXISTS receipts (
        ...
        entry_type VARCHAR(30) NOT NULL DEFAULT 'receipt',
        ...
        UNIQUE KEY uq_receipts_entry (tenant_id, entry_id),
        UNIQUE KEY uq_receipts_no (tenant_id, document_no),
        KEY ix_receipts_date (tenant_id, document_date, id),
        KEY ix_receipts_party (tenant_id, party_name),
        KEY ix_receipts_status (tenant_id, status, is_active)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS receipt_items (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_receipt_items_line (tenant_id, entry_id, line_id),
        KEY ix_receipt_items_parent (tenant_id, entry_id, sort_order)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS receipt_allocations (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_receipt_allocations_allocation (tenant_id, entry_id, allocation_id),
        KEY ix_receipt_allocations_parent (tenant_id, entry_id, sort_order),
        KEY ix_receipt_allocations_document (tenant_id, document_type, document_no)
      )
    `);
    ...
    await db.execute(`
      CREATE TABLE IF NOT EXISTS receipt_comments (
        ...
        body TEXT NOT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uq_receipt_comments_comment (tenant_id, entry_id, comment_id),
        KEY ix_receipt_comments_parent (tenant_id, entry_id, id)
      )
    `);

    await db.execute(`
      CREATE TABLE IF NOT EXISTS receipt_activities (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
        tenant_id VARCHAR(120) NOT NULL,
        entry_id VARCHAR(80) NOT NULL,
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uq_receipt_activities_activity (tenant_id, entry_id, activity_id),
        KEY ix_receipt_activities_parent (tenant_id, entry_id, id)
      )
    `);

    await db.execute(`
      CREATE TABLE IF NOT EXISTS receipt_compliances (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
        tenant_id VARCHAR(120) NOT NULL,
        entry_id VARCHAR(80) NOT NULL,
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uq_receipt_compliances_operation (tenant_id, entry_id, operation_id),
        KEY ix_receipt_compliances_parent (tenant_id, entry_id, id),
        KEY ix_receipt_compliances_provider (provider, environment, operation, status)
      )
    `);
  },
};

export const receiptMigrations = [receiptEntryStorageMigration];
