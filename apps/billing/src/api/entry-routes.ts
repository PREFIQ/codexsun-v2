...
export async function registerEntryRoutes(app: FastifyInstance, ctx: BillingRouteContext) {
  for (const basePath of ["/billing/entries", "/core/entries"]) {
    app.get(`${basePath}/:kind`, async (request) => {
    ...
    app.get(`${basePath}/:kind/:id`, async (request) => {
      const session = await requireTenantSession(app, request, ctx);
      ctx.guardPermission(session, "core.common.view");
    ...
    app.post(`${basePath}/:kind/upsert`, async (request) => {
    ...
    app.post(`${basePath}/:kind/:id/archive`, async (request) => {
      const session = await requireTenantSession(app, request, ctx, true);
      ctx.guardPermission(session, "core.common.manage");
      const { kind: rawKind, id } = request.params as { kind: string; id: string };
    ...
    app.post(`${basePath}/:kind/:id/restore`, async (request) => {
      const session = await requireTenantSession(app, request, ctx, true);
      ctx.guardPermission(session, "core.common.manage");
      const { kind: rawKind, id } = request.params as { kind: string; id: string };
    ...
    app.post(`${basePath}/:kind/:id/comments`, async (request) => {
      const session = await requireTenantSession(app, request, ctx, true);
      ctx.guardPermission(session, "core.common.manage");
      const { kind: rawKind, id } = request.params as { kind: string; id: string };
    ...
    app.post(`${basePath}/:kind/:id/tools`, async (request) => {
      const session = await requireTenantSession(app, request, ctx, true);
      ctx.guardPermission(session, "core.common.manage");
      const { kind: rawKind, id } = request.params as { kind: string; id: string };
      const kind = parseEntryKind(rawKind);
    ...
    app.post(`${basePath}/:kind/:id/compliance/:action`, async (request) => {
      const session = await requireTenantSession(app, request, ctx, true);
      ctx.guardPermission(session, "core.common.manage");
      const { kind: rawKind, id, action } = request.params as { kind: string; id: string; action: string };
      const kind = parseEntryKind(rawKind);
      if (!getBillingEntryModule(kind).supportsCompliance) throw AppError.validation("Compliance generation is available for sales-style entries only");
      const entry = await getEntry(app, request.tenantId!, kind, id);
...
async function requireTenantSession(app: FastifyInstance, request: FastifyRequest & { tenantId?: string }, ctx: BillingRouteContext, mutate = false) {
...
async function upsertEntry(app: FastifyInstance, tenantId: string, kind: EntryKind, input: EntryInput, actorEmail: string) {
  ...
  if (getBillingEntryModule(kind).requiresLines && lines.length === 0) {
...
async function nextDocumentNo(app: FastifyInstance, tenantId: string, kind: EntryKind) {
  const prefix = getBillingEntryModule(kind).documentPrefix;
...
function parseEntryKind(value: string): EntryKind {
  const moduleDefinition = findBillingEntryModule(value);
  if (!moduleDefinition || moduleDefinition.status !== "active") throw AppError.notFound("Entry module not found");
  return moduleDefinition.kind;
}
...
function entryLabel(kind: EntryKind) {
  return getBillingEntryModule(kind).label;
}

function partyLabel(kind: EntryKind) {
  return getBillingEntryModule(kind).partyLabel;
}

function defaultPartyType(kind: EntryKind) {
  return getBillingEntryModule(kind).defaultPartyType;
}
