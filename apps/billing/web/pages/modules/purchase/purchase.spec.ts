...
test.describe("Billing Purchase module", () => {
  test("opens Purchase list through the module-owned page", async ({ page }) => {
    const browserErrors: string[] = [];
    collectBrowserErrors(page, browserErrors);

    await loginAsTenant(page);
    await page.goto("/tenant/entries/purchase");
    ...
    await expect(page.getByText("Create and review purchase bills.")).toBeVisible();
    await expect(page.getByRole("button", { name: "Purchase" }).first()).toBeVisible();
