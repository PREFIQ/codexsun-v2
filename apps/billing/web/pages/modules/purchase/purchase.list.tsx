...
import {
  archivePurchaseEntry,
  commentOnPurchaseEntry,
  listPurchaseEntries,
  restorePurchaseEntry,
  runPurchaseEntryTool,
...
type PurchaseListView =
  | { mode: "list" }
  | { mode: "show"; entry: PurchaseEntry }
  | { mode: "upsert"; entry: PurchaseEntry | null };

export function PurchaseListPage() {
  const meta = entryMeta.purchase;
  const queryClient = useQueryClient();
  const [view, setView] = useState<PurchaseListView>({ mode: "list" });
  ...
  const archiveMutation = useMutation({
    mutationFn: archivePurchaseEntry,
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "purchase"] });
      setView({ mode: "list" });
    },
    onError: (error) => toast.error("Purchase archive failed", { description: apiErrorText(error, "Could not suspend purchase entry") }),
  });
  const restoreMutation = useMutation({
    mutationFn: restorePurchaseEntry,
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "purchase"] });
      setView({ mode: "show", entry: result.entry });
    },
    onError: (error) => toast.error("Purchase restore failed", { description: apiErrorText(error, "Could not restore purchase entry") }),
  });
  const commentMutation = useMutation({
    mutationFn: ({ body, entry }: { body: string; entry: PurchaseEntry }) => commentOnPurchaseEntry(entry, body),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "purchase"] });
      setView({ mode: "show", entry: result.entry });
    },
  ...
  const toolMutation = useMutation({
    mutationFn: ({ entry, tool, value }: { entry: PurchaseEntry; tool: string; value?: string }) => runPurchaseEntryTool(entry, tool, value),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "purchase"] });
      setView({ mode: "show", entry: result.entry });
    },
    onError: (error) => toast.error("Purchase tool failed", { description: apiErrorText(error, "Could not run purchase tool") }),
  });
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((entry) => {
      if (entry.entryType !== "purchase") return false;
  ...
  if (view.mode === "show") {
    ...
    return (
      <PurchaseShowPage
        entry={entry as PurchaseEntryRecord}
        kind="purchase"
  ...
  if (view.mode === "upsert") {
    return (
      <PurchaseFormPage
        entry={view.entry as PurchaseEntryRecord | null}
        existingEntries={entries as PurchaseEntryRecord[]}
        onBack={() => setView(view.entry ? { mode: "show", entry: view.entry } : { mode: "list" })}
        onSaved={(entry) => setView({ mode: "show", entry: entry as PurchaseEntry })}
      />
    );
  }
      ...
      <div className="overflow-hidden rounded-md border border-border/70 bg-card/95 shadow-sm">
        ...
        {!pageItems.length ? (
          <div className="px-6 py-14 text-center text-sm text-muted-foreground">
            {entriesQuery.isLoading ? "Loading purchase..." : "No purchase entries found."}
          </div>
        ) : null}
      </div>
      <WorkspacePagination
        ...
        singularLabel="purchase entries"
