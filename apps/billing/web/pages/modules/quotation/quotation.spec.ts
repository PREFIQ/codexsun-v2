...
test.describe("Billing Quotation module", () => {
  test("opens Quotation list through the module-owned page", async ({ page }) => {
    const browserErrors: string[] = [];
    collectBrowserErrors(page, browserErrors);

    await loginAsTenant(page);
    await page.goto("/tenant/entries/quotation");
    ...
    await expect(page.getByText("Create and review customer quotation entries.")).toBeVisible();
    await expect(page.getByRole("button", { name: "Quotation" }).first()).toBeVisible();
