...
type PaymentListView =
  | { mode: "list" }
  | { mode: "show"; entry: PaymentEntry }
  | { mode: "upsert"; entry: PaymentEntry | null };

export function PaymentListPage() {
  const meta = entryMeta.payment;
  const queryClient = useQueryClient();
  const [view, setView] = useState<PaymentListView>({ mode: "list" });
  ...
  const archiveMutation = useMutation({
    mutationFn: archivePaymentEntry,
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "payment"] });
      setView({ mode: "list" });
    },
    onError: (error) => toast.error("Payment archive failed", { description: apiErrorText(error, "Could not suspend payment entry") }),
  });
  const restoreMutation = useMutation({
    mutationFn: restorePaymentEntry,
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "payment"] });
      setView({ mode: "show", entry: result.entry });
    },
    onError: (error) => toast.error("Payment restore failed", { description: apiErrorText(error, "Could not restore payment entry") }),
  });
  const commentMutation = useMutation({
    mutationFn: ({ body, entry }: { body: string; entry: PaymentEntry }) => commentOnPaymentEntry(entry, body),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "payment"] });
      setView({ mode: "show", entry: result.entry });
    },
  ...
  const toolMutation = useMutation({
    mutationFn: ({ entry, tool, value }: { entry: PaymentEntry; tool: string; value?: string }) => runPaymentEntryTool(entry, tool, value),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "payment"] });
      setView({ mode: "show", entry: result.entry });
    },
    onError: (error) => toast.error("Payment tool failed", { description: apiErrorText(error, "Could not run payment tool") }),
  });
  ...
  if (view.mode === "show") {
    ...
    return (
      <PaymentShowPage
        entry={entry as PaymentEntryRecord}
        kind="payment"
  ...
  if (view.mode === "upsert") {
    return (
      <PaymentFormPage
        entry={view.entry as PaymentEntryRecord | null}
        existingEntries={entries as PaymentEntryRecord[]}
        onBack={() => setView(view.entry ? { mode: "show", entry: view.entry } : { mode: "list" })}
        onSaved={(entry) => setView({ mode: "show", entry: entry as PaymentEntry })}
      />
    );
  }
      ...
      <div className="overflow-hidden rounded-md border border-border/70 bg-card/95 shadow-sm">
        ...
        {!pageItems.length ? (
          <div className="px-6 py-14 text-center text-sm text-muted-foreground">
            {entriesQuery.isLoading ? "Loading payment..." : "No payment entries found."}
          </div>
        ) : null}
      </div>
      <WorkspacePagination
