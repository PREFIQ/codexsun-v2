...
test.describe("Billing Payment module", () => {
  test("opens Payment list through the module-owned page", async ({ page }) => {
    const browserErrors: string[] = [];
    collectBrowserErrors(page, browserErrors);

    await loginAsTenant(page);
    await page.goto("/tenant/entries/payment");

    await expect(page.getByRole("heading", { name: "Payment" })).toBeVisible();
    await expect(page.getByRole("button", { name: "New Payment" })).toBeVisible();
    await expect(page.getByText("Record supplier payments and allocations.")).toBeVisible();
