...
import { SalesFormPage } from "./sales.form";
import {
  archiveSalesEntry,
  commentOnSalesEntry,
  listSalesEntries,
...
type SalesListView =
  | { mode: "list" }
  | { mode: "show"; entry: SalesEntryRecord }
  | { mode: "upsert"; entry: SalesEntryRecord | null };

export function SalesListPage() {
  const meta = entryMeta.sales;
  const queryClient = useQueryClient();
  const [view, setView] = useState<SalesListView>({ mode: "list" });
  ...
  const archiveMutation = useMutation({
    mutationFn: archiveSalesEntry,
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "sales"] });
      setView({ mode: "list" });
    },
    onError: (error) => toast.error("Sales archive failed", { description: apiErrorText(error, "Could not suspend sales entry") }),
  });
  const restoreMutation = useMutation({
    mutationFn: restoreSalesEntry,
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "sales"] });
      setView({ mode: "show", entry: result.entry as SalesEntryRecord });
    },
    onError: (error) => toast.error("Sales restore failed", { description: apiErrorText(error, "Could not restore sales entry") }),
  });
  const commentMutation = useMutation({
    mutationFn: ({ body, entry }: { body: string; entry: SalesEntryRecord }) => commentOnSalesEntry(entry, body),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "sales"] });
      setView({ mode: "show", entry: result.entry as SalesEntryRecord });
    },
  ...
  const toolMutation = useMutation({
    mutationFn: ({ entry, tool, value }: { entry: SalesEntryRecord; tool: string; value?: string }) => runSalesEntryTool(entry, tool, value),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant", "entries", "sales"] });
      setView({ mode: "show", entry: result.entry as SalesEntryRecord });
    },
    onError: (error) => toast.error("Sales tool failed", { description: apiErrorText(error, "Could not run sales tool") }),
  });

  const entries = (entriesQuery.data ?? []) as SalesEntryRecord[];
  const filtered = useMemo(() => {
    const term = searchValue.trim().toLowerCase();
    return entries.filter((entry) => {
      if (entry.entryType !== "sales") return false;
  ...
  if (view.mode === "show") {
    const entry = (entries.find((item) => item.entryId === view.entry.entryId) ?? view.entry) as SalesEntryRecord;
    return (
      <SalesShowPage
        entry={entry}
        kind="sales"
  ...
  if (view.mode === "upsert") {
    return (
      <SalesFormPage
        entry={view.entry}
        existingEntries={entries as SalesEntryRecord[]}
        onBack={() => setView(view.entry ? { mode: "show", entry: view.entry } : { mode: "list" })}
        onSaved={(entry) => setView({ mode: "show", entry })}
      />
    );
  }
      ...
      <div className="overflow-hidden rounded-md border border-border/70 bg-card/95 shadow-sm">
        ...
        {!pageItems.length ? (
          <div className="px-6 py-14 text-center text-sm text-muted-foreground">
            {entriesQuery.isLoading ? "Loading sales..." : "No sales entries found."}
          </div>
        ) : null}
      </div>
      <WorkspacePagination
